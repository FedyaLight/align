...
4. AAF — осознанно НЕ делаем сейчас: `oximedia-aaf` 0.2.1 заявляет stable
   writer и external references, но опубликованный `writer.rs` прямо
   называет сериализацию simplified и записывает только счётчики mobs,
   не сами mobs/slots/locators. Resolve/Avid не смогли бы прилинковать
   медиа — правдоподобный, но нерабочий interchange мы не выпускаем.
   Плюс транзитивные зависимости ради нуля проверяемого функционала
   (живого Avid для проверки нет).
   Вернуться, когда writer дозреет или появится NLE-фиxture.
